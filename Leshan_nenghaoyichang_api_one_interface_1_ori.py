...
df = pd.DataFrame(columns=['floor_id','datetime', 'power_consumption', 'power_consumption_ori', 'predict_value', 'error', 'flag'])
dict_of_list = {}
n_id = 2
...
def anomaly_detection(a, df,floor_id):
    ...
    df_real = pd.DataFrame(list,
                           columns=['floor_id','datetime', 'power_consumption', 'power_consumption_ori', 'predict_value', 'error',
                                    'flag'])
    ...
    if error > 30:
        ...
        print('Warning！{0}节点能耗异常：{1}的用电量真实值{2}与模型预测值{3}的偏差为{4}%，使用预测值作为训练数据'.format(floor_id,df_real.iloc[0].ds, df_real.iloc[0].y,
        ...
        error = '{:.2f}'.format(error)
        predict_value_str = '{:.2f}'.format(predict_value)
        print('{0}节点{1}的用电量真实值{2}与模型预测值{3}的偏差为{4}%,能耗正常。'.format(floor_id,df_real.iloc[0].ds, df_real.iloc[0].y, predict_value_str,
                                                            error))
    ...
    # df.to_json(jsonfile, orient='index', date_format="epoch", date_unit="s")
    # jsonfile.close()
    dict = {'floor_id':df.iat[-1,0],'datetime':df.iat[-1,1],'power_consumption':df.iat[-1,2],'power_consumption_ori':df.iat[-1,3],'predict_value':predict_value, 'error': error,'flag': flag}
    return dict
...
def one_hour_date(floor_id):
    ...
    if (int(a) < 240):
        ...
        print('*********************a + floor_id + current_process():',str(a)+' '+str(floor_id) +str(multiprocessing.current_process()))
        return 'a'
